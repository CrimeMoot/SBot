...
class Tools(commands.Cog):
    ...
    async def status(self, ctx, cat='ы', *, name='Selfbot'):
        response = requests.get('https://discord.com/api/users/@me/settings',
        ...
        if cat in game:
            ...
            await self.bot.change_presence(status=sstatus,
                                           activity=discord.Activity(type=discord.ActivityType.watching, name=name))
        elif cat in listening:
            await self.bot.change_presence(status=sstatus,
                                           activity=discord.Activity(type=discord.ActivityType.listening, name=name))
        elif cat in streaming:
            await self.bot.change_presence(status=sstatus, activity=discord.Streaming(name=name,
                                                                                      url="https://www.youtube.com/watch?v=yNIQi6cbk2s"))
        elif cat in reset:
    ...
    @commands.command(
        aliases=['copy_status', 'copyactivity', 'copy_activity', 'statuscopy', 'status_copy', 'activitycopy',
                 'activity_copy'])
    ...
    async def nitro(self, ctx, amount: int = 100_000, nitrotype='classic'):
        ...
        with open(f'{amount}_nitro_codes.txt', 'w', encoding='utf-8') as f:
            ...
            f.write('--------------------------------------------')
        await ctx.send(f'**:crown: Успешно сгенерировал {amount} нитро кодов!**',
                       file=discord.File(f'{amount}_nitro_codes.txt'))
        await ctx.message.delete()
    ...
    async def deletedms(self, ctx, name='spam'):
        ...
        for dm in self.bot.private_channels:
            if name.lower() in str(dm).lower():
                while True:
                    response = requests.delete(f"https://discord.com/api/v9/channels/{dm.id}",
                                               headers={'authorization': self.bot.http.token})
